    ...
    for m = 1:length(Node_hty)
        for n  = 1:length(Node_hty{m,1})
            ...
            if comp_hty(m,1) == ud
                ...
                htyUd_Name = htyUd(FormatY+1:end);                if htyUd_Name == NodeNber{end,1}(FormatY+1:end)
...
if (mat_change == 1) && (flag==0)
    ...
    fprintf(fileID, '%s\r\n', LineText); 
    LineText = strcat(num2str(t_Hres),','); 
    fprintf(fileID, '%s\r\n', LineText);
    ...
    fprintf(fileID, '%s\r\n', LineText);
    LineText = '*Solid Section, elset=ERESIN, orientation=Ori-3, material=RESINMATERIAL2'; 
    fprintf(fileID, '%s\r\n', LineText);
    LineText = strcat(num2str(t_Vres),','); 
...
fprintf(fileID, '%s\r\n', LineText);
LineText = '*Enrichment, name=Crack-5, type=PROPAGATION CRACK, elset=REPAIREDPANEL-1.ERESIN'; 
fprintf(fileID, '%s\r\n', LineText);
...
fprintf(fileID, '%s\r\n', LineText);
LineText = '*Enrichment, name=Crack-6, type=PROPAGATION CRACK, elset=REPAIREDPANEL-1.ERESIN'; 
fprintf(fileID, '%s\r\n', LineText);
...
fprintf(fileID, '%s\r\n', LineText);
%Knn Kss and Ktt for cohesive elements.
...
% for cohesive resin and the material properties are the same.
LineText =  strcat(num2str(1e9),',',num2str(1e9),',',num2str(1e9),',',num2str(0.38),',',num2str(0.38),',',num2str(0.38),',',num2str(1e9/(2*(1+0.38))),',',num2str(1e9/(2*(1+0.38)))); 
...
fprintf(fileID, '%s\r\n', LineText);
%LineText = strcat(num2str(I_o),',',num2str(I_R),',','9',',','32');
